 ...
 */
void lcd_write_char(uint16_t x, uint16_t y, char ch, FontDef font, uint16_t color,
                    uint16_t bgcolor) {
    uint32_t i, b, j;
 ...
 * display string
 */
void lcd_write_string(uint16_t x, uint16_t y, char *str, FontDef font, uint16_t color,
                      uint16_t bgcolor) {
    while (*str) {
...
void lcd_display(uint8_t symbol) {
    switch (symbol) {
        case 0: lcd_display_cpuLoad(); break;
        case 1: lcd_display_ram(); break;
        case 2: lcd_display_temp(); break;
        case 3: lcd_display_disk(); break;
        default: break;
    }
}
...
void lcd_display_cpuLoad(void) {
    ...
    if (IP_SWITCH == IP_DISPLAY_OPEN) {
        ...
        get_ip_address(iPSource);  // Get the IP address of the device's wireless network card
        lcd_write_string(24, 0, iPSource, Font_8x16, ST7735_WHITE,
                         ST7735_BLACK);  // Send the IP address to the lower machine
    } else {
        lcd_write_string(0, 0, CUSTOM_DISPLAY, Font_8x16, ST7735_WHITE,
                         ST7735_BLACK);  // Send the IP address to the lower machine
    }
